...
background_image = pygame.transform.scale(pygame.image.load("LA.jpg"), (SCREEN_WIDTH, SCREEN_HEIGHT))

font = pygame.font.SysFont("Times New Roman", FONT_SIZE)

# Define a custom event for color change
    ...
    def change_color(self):
        ...
        new_color = random.choice(
            [pygame.Color('green'), pygame.Color('yellow'),
             pygame.Color('purple'), pygame.Color('orange')]
        )
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_x):
            ...
            # Trigger both sprites to change color
    ...
    if not won:
        ...
        if sprite1.rect.colliderect(sprite2.rect):
            # Post custom event instead of removing sprite
